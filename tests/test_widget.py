...
def test_mask_account_card_bad_types():
    with pytest.raises(TypeError):
        mask_account_card(None)
    with pytest.raises(TypeError):
        mask_account_card(12345)

def test_mask_account_card_bad_values():
...
@pytest.mark.parametrize("bad", [None, 12345, 12.34, ["2019-07-03T18:35:29.512364"], {"date": "2019-07-03T18:35:29.512364"}])
def test_get_date_typeerror(bad):
    with pytest.raises(TypeError):
        get_date(bad)

@pytest.mark.parametrize("bad_str", ["", "not a date", "2019-07-03T18:35", "2019/07/03 18:35:29"])
def test_get_date_valueerror(bad_str):
    with pytest.raises(ValueError):
        get_date(bad_str)
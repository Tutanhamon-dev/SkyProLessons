...
def test_filter_by_state(sample_dictionary_list):
    modified_list = filter_by_state(sample_dictionary_list)  # значение ключа "state" по умолчанию "EXECUTED"
...
def test_filter_by_state_case_insensitive(sample_dictionary_list):
    modified_list = filter_by_state(sample_dictionary_list, "canceled")
...
def test_filter_by_state_mixed_list():
    mixed_list = [
        {"id": 1, "state": "EXECUTED"},
    ...
    modified_list = filter_by_state(mixed_list, "EXECUTED")
...
def test_filter_by_state_type_error(sample_dictionary_list):
    # state не строка
...
def test_sort_by_date_descending(sample_dictionary_list):
    modified_list = sort_by_date(sample_dictionary_list)
...
def test_sort_by_date_ascending(sample_dictionary_list):
    modified_list = sort_by_date(sample_dictionary_list, False)
    assert {i["id"] for i in modified_list} == {939719570, 594226727, 939735570, 615064591, 41428829}

def test_sort_by_date_bad_date(sample_dictionary_list):
    # Добавляем в список элементы с не корректным значением 'date'
    missing1 = {"id": 100, "state": "UNKNOWN"}                      # словарь без 'date'
    missing2 = "not a dict"                                         # не словарь
    missing3 = {"id": 200, "state": "EXECUTED", "date": 12345}      # date не строка
    mixed_dictionary_list = [missing1, missing2] + sample_dictionary_list + [missing3]
    # элементы с некорректной 'date' должны оказаться в конце
    modified_list = sort_by_date(mixed_dictionary_list)
